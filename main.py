...
def train(train_loader, model, criterion, optimizer, epoch, log):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        for name, m in model.named_modules():
            if ("conv" in name) or ("classifier" in name) or ("fc" in name) or ("downsample.0" in name):
                if args.input_M2D != 0.0:
                    MSE_credit += (m.weight.data/m.step_size - get_centroid(m.weight.data/m.step_size, m.grain_size, m.num_bits, m.M2D, m.half_lvls)).norm(2)
                    # MSE_credit += (m.weight.data - get_centroid(m.weight.data, m.grain_size, m.num_bits, m.M2D, m.half_lvls)).norm(2)
                    # print("Add_Regularization Term")
                    # weight_d =  (m.weight.data - get_centroid(m.weight.data, m.grain_size, m.num_bits, m.M2D, m.half_lvls)).norm(2)
                    # clipper = (1-m.M2D) * m.half_lvls
                    # MSE_credit += weight_d.clamp_(-clipper, clipper).round_()
        penalty_loss = args.regular_factor * MSE_credit
        entropy_loss = criterion(output, target)
        loss = penalty_loss + entropy_loss
        # loss = criterion(output, target)
        ...
        if MSE_credit != 0:
            reguL.update(penalty_loss.item(), input.size(0))
        ...
        if i % args.print_freq == 0:
            print_log('  Epoch: [{:03d}][{:03d}/{:03d}]   '
                      ...
                      'Regularzation_Loss {reguL.val:.3f} ({reguL.avg:.3f})   '.format(
                          epoch, i, len(train_loader), batch_time=batch_time,
                          data_time=data_time, loss=losses, top1=top1, top5=top5, reguL=reguL) + time_string(), log)
    print_log(
