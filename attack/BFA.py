...
class BFA(object):
    ...
    def flip_bit(self, name, m, update_mask = False, stats = False, restrict_MSB = False):
        '''
        ...
        if restrict_MSB: # restricted cannot attack MSB (choice 1)
            _, b_grad_max_idx = b_grad_topk[1:8].abs().view(-1).topk(self.n_bits2flip)
        else: # no restriction on where to attack (choice 2)
        ...
        if grad_max.item() != 0:  # ensure the max grad is not zero
            ...
            # print(bit2flip.size())
            # bit2flip[[0, 1]] = bit2flip[[1, 0]]
            # bit2flip[[1, 0]].zero_()
        else:
            pass

        # switch MSB and MSB-1 position: (attack MSB-1 instead)
        # print("Before is ")
        # print(bit2flip.short().nonzero()[0])

        # print("After is ")
        # print(bit2flip.short().nonzero()[0])
        flipped_index = bit2flip.short().nonzero()[0][1]
        ...
        if stats:
            ...
            # if (str(flipped_index.item()) + '_' + str(attack_weight_change.item())) in self.attack_index.keys():
                # self.attack_index[name + '_' + (str(flipped_index.item()) + '_' + str(attack_weight_change.item()))] += 1
            # else:
                # self.attack_index[name + '_' + (str(flipped_index.item()) + '_' + str(attack_weight_change.item()))] = 1
            # if (str(flipped_index.item()) + '_' + str(attack_weight_change.item())) in self.attack_index.keys():
            ...
            if not name+'_G4' in self.attack_index.keys():
                self.attack_index[name+'_G4'] = {}
            if not name+'_G8' in self.attack_index.keys():
                self.attack_index[name+'_G8'] = {}
            if not name+'_G16' in self.attack_index.keys():
                self.attack_index[name+'_G16'] = {}
            if not name+'_G32' in self.attack_index.keys():
                self.attack_index[name+'_G32'] = {}
            if not name+'_G64' in self.attack_index.keys():
                self.attack_index[name+'_G64'] = {}
            if str(int(flipped_index.item()/4)) in self.attack_index[name+'_G4'].keys():
                self.attack_index[name+'_G4'][str(int(flipped_index.item()/4))] += 1
            else:
                self.attack_index[name+'_G4'][str(int(flipped_index.item()/4))] = 1

            if str(int(flipped_index.item()/8)) in self.attack_index[name+'_G8'].keys():
                self.attack_index[name+'_G8'][str(int(flipped_index.item()/8))] += 1
            else:
                self.attack_index[name+'_G8'][str(int(flipped_index.item()/8))] = 1

            if str(int(flipped_index.item()/16)) in self.attack_index[name+'_G16'].keys():
                self.attack_index[name+'_G16'][str(int(flipped_index.item()/16))] += 1
            else:
                self.attack_index[name+'_G16'][str(int(flipped_index.item()/16))] = 1

            if str(int(flipped_index.item()/32)) in self.attack_index[name+'_G32'].keys():
                self.attack_index[name+'_G32'][str(int(flipped_index.item()/32))] += 1
            else:
                self.attack_index[name+'_G32'][str(int(flipped_index.item()/32))] = 1
    ...
    def progressive_bit_search(self, model, data, target, update_mask_flag = False, save_stats = False, twin = False, lessen = False):
        ...
        print("lessen is set to ", lessen)
        print("twin attack is set to ", twin)
        for name, module in model.named_modules():
            # print(name)
            if name == max_loss_module:
                ...
                if lessen == True:
                    ...
                    if reshaped_attack_weight[neighbor_index] > 0:
                        reshaped_attack_weight[neighbor_index] -= 192
                    else:
                        reshaped_attack_weight[neighbor_index] += 192
