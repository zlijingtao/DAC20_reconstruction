...
import math
import numpy as np
from .fixpoint_modules import  get_centroid, get_quantized, quantize
from tensorboardX import SummaryWriter
from .check_modules import get_qcode, get_code_2, get_mean_bits, get_mean, get_rescue
from os import path
import config
import random
# Setup tensorboard, change 12 to any other number to begin a new recording
writer = SummaryWriter('runs/12/signature_data_0')
class Unite(torch.autograd.Function):
    ...
    def forward(self, input):
        ...
        #Set rescue to True if want to zero out detected region
        rescue = True
        #Set con_clip to True if want to simulate what if half of the case I didn't detect correctly
        coin_flip = False
        '''Run Checker'''
        ...
        if not(path.exists(self.save_path + '/saved_tensors/signature{}.pt'.format(ti))):
            ...
            if detected_item != 0:
                ...
                if rescue:
                    #check unmatched signature to locate faulty region
                    wrong_mask = (signature - orig_signature == 0)
                    if coin_flip == True:
                        wrong_mask[:int(wrong_mask.size()[0]/2)] = 1
                    input_d = get_rescue(input_d, [1,self.checking_group], wrong_mask, self.checking_bit)
