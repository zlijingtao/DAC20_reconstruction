...
class Unite(torch.autograd.Function):
    ...
    def forward(self, input):
        ...
        # signiture = get_mean_bits(input, self.grain_size, 2, self.half_lvls)
        # count = (input_d - self.W > 0).sum() + (input_d + self.W < 0).sum()
        # if not(path.exists(self.save_path + '/saved_tensors/signiture{}.pt'.format(ti))):
            # if ti <=num_res:
                # torch.save(signiture, self.save_path + '/saved_tensors/signiture{}.pt'.format(ti))
...
class quan_Conv2d(nn.Conv2d):
    ...
    def forward(self, input, dual_module = False, running_mean = 0, running_var = 0, beta = 0, gamma = 0):
        test_activation = F.conv2d(input, self.weight * self.step_size, self.bias,
                self.stride, self.padding, self.dilation,
                self.groups)
        correct_shape = test_activation.size()
        unfold_input = torch.nn.functional.unfold(input = input, kernel_size=self.kernel_size, dilation = self.dilation, padding = self.padding, stride = self.stride)
        if self.inf_with_weight:
            ...
            centroid_activation = unfold_input.transpose(1, 2).matmul(self.step_size * weight_centroid.view(weight_centroid.size(0), -1).t()).transpose(1,2)
            dev_activation = unfold_input.transpose(1, 2).matmul(self.step_size * weight_dev.view(weight_dev.size(0), -1).t()).transpose(1,2)
            centroid_activation = centroid_activation.view(correct_shape)
            dev_activation = dev_activation.view(correct_shape)
            ...
            # full_activation = full_activation.view(correct_shape)
            ...
            centroid_activation = unfold_input.transpose(1, 2).matmul(weight_centroid.view(weight_centroid.size(0), -1).t()).transpose(1,2)
            dev_activation = unfold_input.transpose(1, 2).matmul(weight_dev.view(weight_dev.size(0), -1).t()).transpose(1,2)
            centroid_activation = centroid_activation.view(correct_shape)
            dev_activation = dev_activation.view(correct_shape)
            if dual_module == True:
                full_activation = get_aggregate(centroid_activation, dev_activation, running_mean, running_var, gamma, beta)
...
class CifarResNet(nn.Module):
  ...
  def forward(self, x):
    x = self.conv_1_3x3(x, dual_module = True)
...
def dual_quan_resnet20(num_classes=10, input_grain_size = (1,1), input_num_bits = 4, input_M2D = 0.0, res_grain_size = (1,1), res_num_bits = 4, res_M2D = 0.0, output_grain_size = (1,1), output_num_bits = 4, output_M2D = 0.0, save_path = './'):
...
def dual_quan_resnet32(num_classes=10, input_grain_size = (1,1), input_num_bits = 4, input_M2D = 0.0, res_grain_size = (1,1), res_num_bits = 4, res_M2D = 0.0, output_grain_size = (1,1), output_num_bits = 4, output_M2D = 0.0, save_path = './'):
...
def dual_quan_resnet44(num_classes=10, input_grain_size = (1,1), input_num_bits = 4, input_M2D = 0.0, res_grain_size = (1,1), res_num_bits = 4, res_M2D = 0.0, output_grain_size = (1,1), output_num_bits = 4, output_M2D = 0.0, save_path = './'):
...
def dual_quan_resnet56(num_classes=10, input_grain_size = (1,1), input_num_bits = 4, input_M2D = 0.0, res_grain_size = (1,1), res_num_bits = 4, res_M2D = 0.0, output_grain_size = (1,1), output_num_bits = 4, output_M2D = 0.0, save_path = './'):
...
def dual_quan_resnet110(num_classes=10, input_grain_size = (1,1), input_num_bits = 4, input_M2D = 0.0, res_grain_size = (1,1), res_num_bits = 4, res_M2D = 0.0, output_grain_size = (1,1), output_num_bits = 4, output_M2D = 0.0, save_path = './'):
