...
class LeNet_5(nn.Module):
    def __init__(self, num_classes, 
        ...
        self.conv1 = BinarizeConv2d(1, 20, kernel_size=5, stride=1, padding=1, bias=False, grain_size = input_grain_size, num_bits = input_num_bits, M2D = input_M2D, save_path = save_path)
        self.bn_conv1 = nn.BatchNorm2d(20, eps=1e-4, momentum=0.1, affine=False)
        self.relu = nn.ReLU(inplace=True)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.bin_conv2 = BinarizeConv2d(20, 50, kernel_size=5, stride=1, padding=0, bias=False, grain_size = res_grain_size, num_bits = res_num_bits, M2D = res_M2D, save_path = save_path)
        self.bn_conv2 = nn.BatchNorm2d(50, eps=1e-4, momentum=0.1, affine=True)
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.bin_ip1 = BinarizeLinear(50*4*4, 500, grain_size = res_grain_size, num_bits = res_num_bits, M2D = res_M2D, save_path = save_path)
        self.bn_ip1 = nn.BatchNorm1d(500, eps=1e-4, momentum=0.1, affine=True)
        self.ip2 = BinarizeLinear(500, num_classes, output_grain_size, output_num_bits, output_M2D, save_path = save_path)
...
def lenet5_bin(num_classes=10, input_grain_size = (1,1), input_num_bits = 4, input_M2D = 0.0, res_grain_size = (1,1), res_num_bits = 4, res_M2D = 0.0, output_grain_size = (1,1), output_num_bits = 4, output_M2D = 0.0, save_path = './'):
