...
class MLP_4(nn.Module):
    def __init__(self, num_classes, 
        ...
        self.fc1 = BinarizeLinear(784, 512, grain_size = input_grain_size, num_bits = input_num_bits, M2D = input_M2D, save_path = save_path)
        self.bn_1 = nn.BatchNorm1d(512, eps=1e-4, momentum=0.15)
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = BinarizeLinear(512, 512, grain_size = res_grain_size, num_bits = res_num_bits, M2D = res_M2D, save_path = save_path)
        self.bn_2 = nn.BatchNorm1d(512, eps=1e-4, momentum=0.15)
        self.fc3 = BinarizeLinear(512, 512, grain_size = res_grain_size, num_bits = res_num_bits, M2D = res_M2D, save_path = save_path)
        self.bn_3 = nn.BatchNorm1d(512, eps=1e-4, momentum=0.15)
        self.classifier = BinarizeLinear(512, num_classes, output_grain_size, output_num_bits, output_M2D, save_path = save_path)
...
def mlp_512_bin(num_classes=10, input_grain_size = (1,1), input_num_bits = 4, input_M2D = 0.0, res_grain_size = (1,1), res_num_bits = 4, res_M2D = 0.0, output_grain_size = (1,1), output_num_bits = 4, output_M2D = 0.0, save_path = './'):
