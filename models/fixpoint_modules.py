...
def get_aggregate(input_c, input_d, running_mean = 0, running_var = 0, gamma = 0, beta = 0, bn = False):
    if len(input_c.size()) == 2:
        input_c[input_c > 0] = input_c[input_c > 0] + input_d[input_c > 0]
        output = input_c
    if len(input_c.size()) == 4:
        if bn == False:
            input_c[input_c > 0] = input_c[input_c > 0] + input_d[input_c > 0]
            output = input_c
        else:
            original_size = input_c.size()
            reshaped_input_c = input_c.permute(1, 2, 3, 0).contiguous().view(original_size[1], -1)
            reshaped_input_d = input_d.permute(1, 2, 3, 0).contiguous().view(original_size[1], -1)
            batched_normed = gamma[:, None] * ((reshaped_input_c - running_mean[:, None])/running_var[:, None]) - beta[:, None]
            reshaped_input_c[batched_normed > 0] = reshaped_input_c[batched_normed > 0] + reshaped_input_d[batched_normed > 0]
            output = reshaped_input_c.view(original_size[1], original_size[2], original_size[3], original_size[0]).contiguous().permute(3, 0, 1, 2)
